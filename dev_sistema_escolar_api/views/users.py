...
class AdminAll(generics.CreateAPIView):
    #Esta función es esencial para todo donde se requiera autenticación de inicio d e sesión 
    permission_classes = (permissions.IsAuthenticated,) 
    def get(self, request, *args, **kwargs):
        admin = Administradores.objects.filter(user__is_active = 1).order_by("id") #Administradores = tabla de administradores (modelo)
        lista = AdminSerializer(admin, many=True).data #Serializamos los datos del administrador para volverlo de nuevo JSON
